...
import { rootFolderId, mainSpreadsheetId, archiveFolderId, audioFolderName, imageFolderName } from '../config/google-drive.js'
import { log } from './log.js'

export async function cleanup() {
	let name = new Date(Date.now() - 24*60*60e3).toISOString().split('T')[0]
	log('Archiving spreadsheet...')
	await copyFile(mainSpreadsheetId, archiveFolderId, name)
